...
const navItems = [
  ...
  { href: "/dashboard/configuracion", label: "Configuracion", icon: "⚙️" },
];

function NavLinks({ items, pendingCount, role, onClose }: { items: typeof navItems; pendingCount: number; role: string; onClose?: () => void }) {
...
export function Sidebar({ pendingCount = 0, role = "ADMIN" }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Mobile drawer */}
      <aside
        className={`lg:hidden fixed top-0 left-0 h-full w-[240px] flex flex-col z-30 transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}
      ...
      >
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          <NavLinks items={navItems} pendingCount={pendingCount} role={role} onClose={() => setMobileOpen(false)} />
        </nav>
      ...
      {/* Desktop sidebar */}
      <aside
        className="hidden lg:flex fixed left-0 top-0 h-full w-[240px] flex-col z-20"
        style={{ background: "#08041A", borderRight: "1px solid rgba(124,58,237,0.15)" }}
      >
        <div className="flex items-center gap-3 px-6 py-5 border-b border-[rgba(124,58,237,0.1)]">
          <EclipseLogo size={36} />
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
          <NavLinks items={navItems} pendingCount={pendingCount} role={role} />
        </nav>
        <div className="px-4 py-4 border-t border-[rgba(124,58,237,0.1)]">
          <p className="text-[10px] text-[#7B6FA0] text-center">Eclipse FM © {new Date().getFullYear()}</p>
        </div>
